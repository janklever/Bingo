...
export default function BingoGlobe({ width, height, isSpinning }) {
  const balls = [
    [80, 128, 15, '#D4A853'],
    [114, 142, 13, '#C8983A'],
    [152, 122, 14, '#D4A853'],
  ...
  return (
    <div className={`globe-wrapper ${isSpinning ? 'spinning' : ''}`}>
      <svg viewBox="0 0 240 300" width={width} height={height}>
        ...
        {balls.map(([cx, cy, r, fill], i) => (
          <circle key={i} cx={cx} cy={cy} r={r} fill={fill} style={{ filter: 'drop-shadow(0 2px 4px rgba(0,0,0,.42))' }} />
        ))}
